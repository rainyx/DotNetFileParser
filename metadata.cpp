...
TableRowIndex Metadata::GetDeclaringType(TableRowIndex nestedTypeDefRowIndex) const {
  const_cast<Metadata *>(this)->BuildDeclaringTypesCache();
  if (_declaringTypes.find(nestedTypeDefRowIndex) != _declaringTypes.end())
    return _declaringTypes.at(nestedTypeDefRowIndex);
  return TableRowIndexInvalid;
}

const std::vector<TableRowIndex> &Metadata::GetGenericParamsForType(TableRowIndex typeDefRowIndex) const {
  const_cast<Metadata *>(this)->BuildGenericParamsCache();
  if (_typeDefGenericParams.find(typeDefRowIndex) != _typeDefGenericParams.end())
    return _typeDefGenericParams.at(typeDefRowIndex);
  return {};
}

const std::vector<TableRowIndex> &Metadata::GetGenericParamsForMethod(TableRowIndex typeDefRowIndex) const {
  const_cast<Metadata *>(this)->BuildGenericParamsCache();
  if (_methodDefGenericParams.find(typeDefRowIndex) != _methodDefGenericParams.end())
    return _methodDefGenericParams.at(typeDefRowIndex);
  return {};
}

const std::vector<TypeDefOrRefCodedIndex> &Metadata::GetInterfaceImplsForType(TableRowIndex typeDefRowIndex) const {
  const_cast<Metadata *>(this)->BuildInterfaceImplsCache();
  if (_interfaceImpls.find(typeDefRowIndex) != _interfaceImpls.end())
    return _interfaceImpls.at(typeDefRowIndex);
  return {};
}


void Metadata::BuildDeclaringTypesCache() {
  if (_declaringTypesCached)
    return;
  _declaringTypesCached = true;

  for (TableRowIndex rowIdx=TableRowIndexStart; rowIdx<=GetNestedClassTable().GetRowsCount(); ++rowIdx) {
    const auto &row = GetNestedClassTable().GetRowAt(rowIdx);
    _declaringTypes.insert({row.NestedClass, row.EnclosingClass});
  }
}

void Metadata::BuildGenericParamsCache() {
  if (_genericParamsCached)
    return;
  _genericParamsCached = true;

  for (TableRowIndex genericParamRowIndex = TableRowIndexStart; genericParamRowIndex <= GetGenericParamTable().GetRowsCount(); ++genericParamRowIndex) {
    const auto &row = GetGenericParamTable().GetRowAt(genericParamRowIndex);
    auto tableIndex = row.Owner.GetTableIndex();
    auto tableRowIndex = row.Owner.GetRowIndex();
    if (tableIndex == TableIndexTypeDef) {
      if (_typeDefGenericParams.find(tableRowIndex) == _typeDefGenericParams.end())
        _typeDefGenericParams.insert({tableRowIndex, vector<TableRowIndex>()});
      _typeDefGenericParams.at(tableRowIndex).push_back(genericParamRowIndex);
    } else if (tableIndex == TableIndexMethodDef) {
      if (_methodDefGenericParams.find(tableRowIndex) == _methodDefGenericParams.end())
        _methodDefGenericParams.insert({tableRowIndex, vector<TableRowIndex>()});
      _methodDefGenericParams.at(tableRowIndex).push_back(genericParamRowIndex);
    }
  }
}

void Metadata::BuildInterfaceImplsCache() {
  if (_interfaceImplsCached)
    return;
  _interfaceImplsCached = true;

  for (TableRowIndex rowIdx = TableRowIndexStart; rowIdx <= GetInterfaceImplTable().GetRowsCount(); ++rowIdx) {
    const auto &row = GetInterfaceImplTable().GetRowAt(rowIdx);
    if (_interfaceImpls.find(row.Class) == _interfaceImpls.end())
      _interfaceImpls.insert({row.Class, vector<TypeDefOrRefCodedIndex>()});

    _interfaceImpls.at(row.Class).push_back(row.Interface);
  }
}
...
//  cout << _metadataHeader->NumberOfStreams << endl;
  ...
  for (int i=0; i<numOfStreams; ++i) {
    ...
    Stream stream = {
      ...
      .Size       = st->Size,
      .Name       = name,
      .Data       = reinterpret_cast<const uint8_t *>(metadataHeader) + st->Offset
    };
//    cout << name << endl;
  ...
  const TableStreamHeader *tableHeader = reinterpret_cast<const TableStreamHeader *>(reinterpret_cast<const uint8_t *>(metadataHeader) + metadata->GetStreamByName("#~").Offset);
